...
void buildgraph(int N){
	...
	while (mididx < jumpsize){
		int s1 = jumplist[mididx].s1;
		int t1 = jumplist[mididx].t1;
		int s2 = jumplist[mididx].s2;
		int t2 = jumplist[mididx].t2;
		vlist[2 * s1 - 1].push_back((vertex){2 * N + mididx + 1, (unsigned long long)jumplist[mididx].t1});
		vlist[2 * N + mididx + 1].push_back((vertex){2 * N + jumpsize + mididx + 1, (unsigned long long)1});
		vlist[2 * N + jumpsize + mididx + 1].push_back((vertex){2 * s2, (unsigned long long)slength[s2] - t2});
		mididx++;
	}
	...
	for (int i = 0; i < jumpsize; i++){
		...
		while (start < jumpsize){
			if (jumplist[start].s1 == jumplist[i].s1 && jumplist[start].t1 > jumplist[i].t1){
				int tlength = jumplist[start].t1 - jumplist[i].t1;
				vlist[2 * N + i + 1].push_back((vertex){2 * N + start + 1, (unsigned long long)tlength});
			}
			if (jumplist[start].s2 == jumplist[i].s1 && jumplist[start].t1 > jumplist[i].t1){
				int tlength = jumplist[start].t1 - jumplist[i].t1;
				vlist[2 * N + i + 1].push_back((vertex){2 * N + start + 1, (unsigned long long)tlength});
			}
			start++;
