...
/*
cited:
	the above code is from the website below:
	http://insilab.org/maxclique/?fbclid=IwAR2iJAX-2LNxb5JikZAzwAc-rbORC__GaKuHSRUczCb_usvB9-ANjh22OcQ
*/

#define NDEBUG
#define MAXN 130
int main(){
	bool **graph = new bool*[MAXN];
	string s;
	int n;
	scanf("%d", &n);
	for (int i = 0; i < n; i++){
...
#endif
//Here use code from others
	Maxclique myclique(graph, n);	
	int* result;
	int resultnum;
	myclique.mcq(result, resultnum);

	int ans[MAXN];
	bool fill[MAXN] = {0};
	bool isclique[MAXN] = {0};
	...
	for (int i = resultnum; i < n; i++){
		while (isclique[count] == 1){
			count++;
		}
		ans[i] = count;
		count++;
